...
<div class="container">
    <div class="caption">@db.matrix</div>
    <div class="wrap overflow">
	<?php echo PYF_DISPLAY_ERROR($err); ?>  
    <table class="tmp-table-compact">
    <?php
		$table = "matrix";
		$limit = 100;
		...
		if ($result->num_rows > 0) 
		{
			...
			$sn = 1;
			 ...
			 { 
			 	...
			 	for ($i = 1; $i <= 24; $i++)
				{
					$buffer_th .= '<th>ROW '.$i.'</th>';					
					$buffer_td .= '<td>'.$row['row'.$i].'</td>';
				}
				 $output .= '<tr>
					<td><a class="tn btn-sec" data-table="'.$table.'" data-id="'.$row['id'].'" onClick="$_smartDelete(this)">Delete</a></td>
					<td>'.$sn.'</td>'.
					$buffer_td.'
					<td>'.$row['status'].'</td>
					<td>'.t_eta($row['eta']).'</td>
